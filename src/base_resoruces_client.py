...
class BaseResourceClient:
    ...
    def get(self, item_id: int) -> Tuple[HTTPStatus, Dict[str, Any]]:
        return self._client.request(HTTPMethod.GET, f"{self._path}/{item_id}", expected={HTTPStatus.OK, HTTPStatus.NOT_FOUND})
    ...
    def update(self, item_id: int, payload: Dict[str, Any]) -> Tuple[HTTPStatus, Dict[str, Any]]:
        return self._client.request(HTTPMethod.PUT, f"{self._path}/{item_id}", json=payload, expected={HTTPStatus.OK, HTTPStatus.NOT_FOUND})

    def delete(self, item_id: int) -> HTTPStatus:
        status, _ = self._client.request(HTTPMethod.DELETE, f"{self._path}/{item_id}", expected={HTTPStatus.NO_CONTENT, HTTPStatus.NOT_FOUND})
        return status


def resource_client_factory(
    cls: Type[BaseResourceClient], client: BaseClient
) -> BaseResourceClient:
    return cls(client)
